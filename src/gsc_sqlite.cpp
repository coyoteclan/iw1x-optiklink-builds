...
struct async_sqlite_task
{
    ...
    int intValue;
    float floatValue;
    char stringValue[MAX_STRING_CHARS];
    vec3_t vectorValue;
...
void *async_sqlite_query_handler(void*)
{
    while(1)
    {
        ...
        while (current != NULL)
        {
            ...
            if (!task->done)
            {
                task->result = sqlite3_prepare_v2(task->db, task->query, MAX_STRING_CHARS, &task->statement, 0);

                if (task->result != SQLITE_OK)
                {
                    task->error = true;
                    strncpy(task->errorMessage, sqlite3_errmsg(task->db), MAX_STRING_CHARS - 1);
                    task->errorMessage[MAX_STRING_CHARS - 1] = '\0';
                }
...
void gsc_async_sqlite_create_query()
{
    ...
    strncpy(newtask->query, query, MAX_STRING_CHARS - 1);
    newtask->query[MAX_STRING_CHARS - 1] = '\0';
...
void gsc_sqlite_query()
{
    ...
    result = sqlite3_prepare_v2((sqlite3 *)db, query, MAX_STRING_CHARS, &statement, 0);
